Create one horizontal animation strip for Codex pet `ga-navigator`, state `running-right`.
...
Identity: same pet in every frame: A tiny original AI navigator mascot with a rounded robot-companion body, subtle GA monogram chest badge, warm off-white shell, black glass faceplate, teal-blue glowing antenna/visor accents, small tool-satchel details, friendly expressive eyes, desktop helper personality; no copyrighted character references, no text except a simple GA badge mark.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
