...
export default function App() {
  ...
  return <div ref={appScope} className={`app app-tab-${tab}`}>
    ...
    <main className="main"><header><div><h2>{t.nav[tab]}</h2><p>{t.desc[tab]}</p></div><div className="badges"><span>{cfg?.host}:{cfg?.port}</span><span role="status" aria-live="polite" className={health?.ok?'ok':'err'}>{health?.ok ? t.ready : t.error}</span></div></header>
      ...
      {tab==='autonomous' && <section><div className="flex" style={{gap:8,alignItems:'center',marginBottom:12,padding:'8px 0',borderBottom:'1px solid #eee'}}><span style={{fontSize:13,color:'#555'}}>{t.fields.llmNo}:</span><select value={autoLLMNo} onChange={e=>setAutoLLMNo(e.target.value)} style={{padding:'4px 8px',borderRadius:6,border:'1px solid #ccc'}}>{profiles.filter(p=>p.type==='chat'||p.type==='api'||p.type==='openai').map(p=><option key={p.name} value={p.name}>{p.name}</option>)}</select><button disabled={busy} onClick={startAutonomous} style={{padding:'4px 12px',background:'#2563eb',color:'#fff',border:'none',borderRadius:6,cursor:busy?'not-allowed':'pointer'}}><Play size={14}/>{t.fields.startGoalMode}</button></div><div className="grid2"><Panel title={t.lists.reflectServices}>{reflectSvcs.length ? reflectSvcs.map(s=><ServiceRow key={s.name} svc={s} t={t} onStart={n=>serviceAction(n,'start')} onStop={n=>serviceAction(n,'stop')} onLogs={viewServiceLogs} onAutostart={toggleServiceAutostart}/>) : <p className="muted">{t.hints.noReflect}</p>}</Panel><Panel title={t.lists.reflectScripts}><EntryList items={inv.reflect || []} empty={t.hints.noReflect}/></Panel></div><Panel title={t.lists.recentReports}><div className="report-list">{(inv.autonomous_reports || []).map(r=><button key={r.path} onClick={()=>readScheduleArtifact(r.path, 'autonomous')}>{r.name}<small>{new Date(r.mod_time).toLocaleString()}</small></button>)}</div><pre className="artifact-view">{scheduleArtifactTitle?.includes('autonomous_reports') ? (scheduleArtifact || t.empty) : t.empty}</pre></Panel></section>}
