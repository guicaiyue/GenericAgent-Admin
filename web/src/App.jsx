...
function HatchPetSettings() {
  ...
  return <Panel title="内置 hatch-pet" className="hatch-pet-card"><div className="hatch-pet-settings"><div><p className="muted">GA Admin 已随程序内置 Codex hatch-pet skill，可在没有外部 .codex skill 目录时一键导出工具链。</p><div className="hatch-pet-meta"><span className="badge ok">{status?.embedded ? '已内置' : '检测中'}</span><span>{fileCount} files</span><span>{totalBytes ? `${Math.round(totalBytes / 1024)} KB` : '—'}</span><span className={exported ? 'badge ok' : 'badge'}>{exported ? '已导出' : '未导出'}</span></div><code className="hatch-pet-path">{exportPath}</code>{status?.error && <p className="err-text">{status.error}</p>}{msg && <p className="muted">{msg}</p>}</div><div className="hatch-pet-actions"><button disabled={busy} onClick={exportSkill}><Download size={14}/>导出到 Codex skills</button><button disabled={busy} onClick={openExportDir}><FolderCog size={14}/>打开导出目录</button><button disabled={busy} onClick={refresh}><RefreshCw size={14}/>刷新</button></div></div></Panel>
}
