...
export default function App() {
  ...
  const [goals, setGoals] = useState([]), [goalObjective, setGoalObjective] = useState(''), [goalBudget, setGoalBudget] = useState(480), [goalMaxTurns, setGoalMaxTurns] = useState(200), [goalLLMNo, setGoalLLMNo] = useState(''), [selectedGoal, setSelectedGoal] = useState(''), [goalOutput, setGoalOutput] = useState(''), [goalOutputMeta, setGoalOutputMeta] = useState(null), [goalPythonPath, setGoalPythonPath] = useState(() => localStorage.getItem('ga-admin-goal-python-path') || '')
  ...
  useEffect(() => { localStorage.setItem('ga-admin-goal-python-path', goalPythonPath) }, [goalPythonPath])
  ...
  const startGoal = async () => {
    ...
    try {
      ...
      const pythonPath = goalPythonPath.trim()
      if (pythonPath) body.python_path = pythonPath
...
function GoalsPage({ t, goals, objective, setObjective, budget, setBudget, maxTurns, setMaxTurns, llmNo, setLLMNo, pythonPath, setPythonPath, outputBytes, setOutputBytes, autoRefresh, setAutoRefresh, selected, output, outputMeta, busy, onStart, onStop, onDelete, onRefresh, onOutput, onClearOutput, onFile, setMsg }) {
      ...
      <div className="goal-left">
        <Panel title={t.fields.startGoalMode} className="goal-start-panel">
          ...
          <div className="form-grid compact-form goal-params">
            ...
            <label>{t.fields.pythonPath}<input value={pythonPath} onChange={e=>setPythonPath(e.target.value)} placeholder={t.fields.pythonAuto}/></label>
          </div>
          <p className="muted goal-python-hint">{t.hints.goalPythonHelp}</p>
