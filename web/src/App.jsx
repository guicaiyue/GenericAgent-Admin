...
const nextThemeMode = value => value === 'system' ? 'dark' : value === 'dark' ? 'light' : 'system'
const isTextInputTarget = target => target?.closest?.('input, textarea, select, [contenteditable="true"]')
...
export default function App() {
  ...
  const [themeMode, setThemeMode] = useState(() => normalizeThemeMode(localStorage.getItem('ga-admin-theme')))
  ...
  useEffect(() => {
    ...
    localStorage.setItem('ga-admin-theme', themeMode)
  ...
  useEffect(() => {
    const onThemeShortcut = event => {
      ...
      setThemeMode(value => nextThemeMode(value))
    }
    window.addEventListener('keydown', onThemeShortcut)
    return () => window.removeEventListener('keydown', onThemeShortcut)
  }, [])
  const t = I18N[lang] || I18N.en
  const themeLabel = themeMode === 'system' ? (lang === 'zh' ? '跟随系统' : 'System') : themeMode === 'dark' ? (lang === 'zh' ? '深色' : 'Dark') : (lang === 'zh' ? '浅色' : 'Light')
  const themeStateLabel = lang === 'zh'
    ? `当前主题：${themeMode === 'system' ? `跟随系统（${effectiveTheme === 'dark' ? '深色' : '浅色'}）` : themeLabel}`
    : `Current theme: ${themeMode === 'system' ? `System (${effectiveTheme})` : themeLabel}`
  const themeActionLabel = lang === 'zh' ? `${themeStateLabel}，点击或按 Alt+T 切换` : `${themeStateLabel}. Click or press Alt+T to switch`
  ...
  const startService = (name) => {
    const svc = services.find(s => s.name === name) || { name }
    if (svc.kind === 'reflect') return openServiceStartDialog(svc)
    return serviceAction(name, 'start')
  }
  ...
  const loadTask = async (id) => { setBusy(true); try { const d = await api(`/api/schedule/task?id=${encodeURIComponent(id)}`); setTaskId(d.id || id); setTaskEditor(safeJson(d.raw)); navigateTo('tasks', 'scheduled') } catch(e){ setMsg(e.message) } finally{ setBusy(false) } }
  const saveTask = async () => { const id = taskId || newTaskId; if (!confirmDanger('schedule-save', `保存定时任务 ${id}？后端会写入 JSON 并生成备份。`)) return; setBusy(true); try { await api('/api/schedule/task', { dangerous:true, method:'PUT', body: JSON.stringify({ id, raw: JSON.parse(taskEditor) }) }); setMsg(t.hints.taskSaved); await load(); navigateTo('tasks', 'scheduled', { replace: true }) } catch(e){ setMsg(e.message) } finally{ setBusy(false) } }
  const createTask = async () => { setTaskId(newTaskId); setTaskEditor(safeJson({ schedule: '09:00', repeat: 'daily', enabled: false, prompt: '' })); navigateTo('tasks', 'scheduled') }
  const deleteTask = async () => { if (!taskId) return; if (!confirmDanger('schedule-delete', `删除定时任务 ${taskId}？后端会先生成备份。`)) return; setBusy(true); try { await api('/api/schedule/delete', { dangerous:true, method:'POST', body: JSON.stringify({ id: taskId }) }); setMsg(t.hints.taskDeleted); setTaskId(''); setTaskEditor('{}'); await load(); navigateTo('tasks', 'scheduled', { replace: true }) } catch(e){ setMsg(e.message) } finally{ setBusy(false) } }
  const readScheduleArtifact = async (path, targetTab = 'tasks') => { setBusy(true); try { const d = await api(`/api/schedule/artifact?path=${encodeURIComponent(path)}`); setScheduleArtifactTitle(path); setScheduleArtifact(d.content || ''); navigateTo(targetTab, 'reports') } catch(e){ setMsg(e.message) } finally{ setBusy(false) } }
    ...
    <main className="main"><header><div><h2>{t.nav[tab]}</h2><p>{t.desc[tab]}</p></div><div className="badges"><span>{cfg?.host}:{cfg?.port}</span><span role="status" aria-live="polite" className={health?.ok?'ok':'err'}>{health?.ok ? t.ready : t.error}</span></div></header>
      ...
      {tab==='tasks' && <section className="tasks-page">
        ...
        <div className="subtabs task-subtabs">
          <button className={taskSubTab==='services' ? 'active' : ''} onClick={()=>navigateTo('tasks', 'services')}><Server size={14}/>{t.lists.taskServices}</button>
          <button className={taskSubTab==='scheduled' ? 'active' : ''} onClick={()=>navigateTo('tasks', 'scheduled')}><CalendarClock size={14}/>{t.lists.scheduledTasks}</button>
          <button className={taskSubTab==='runs' ? 'active' : ''} onClick={()=>navigateTo('tasks', 'runs')}><Target size={14}/>{t.nav.goals} / {t.nav.autonomous}</button>
          <button className={taskSubTab==='reports' ? 'active' : ''} onClick={()=>navigateTo('tasks', 'reports')}><FolderCog size={14}/>{t.lists.recentReports}</button>
        </div>
        ...
        {taskSubTab==='services' && <div className="single-panel">
          <Panel title={t.lists.taskServices}>
            ...
            <div className="service-list clean-list">
              {taskSvcs.length
                ? taskSvcs.map(svc => <ServiceRow key={svc.name} svc={svc} t={t} onStart={startService} onStop={n=>serviceAction(n,'stop')} onLogs={viewServiceLogs} onAutostart={toggleServiceAutostart}/>)
        ...
        {taskSubTab==='runs' && <div className="workspace tasks-workspace">
          <Panel title={`${t.nav.goals} · ${goals.filter(g=>g.running).length}/${goals.length}`}>
            <div className="actions"><button onClick={()=>navigateTo('goals')}><Target size={14}/>{t.nav.goals}</button><button onClick={loadGoals}><RefreshCw size={14}/>{t.refresh}</button></div>
            <div className="goal-list compact-goals">
              {goals.length
                ? goals.map(g => <button className="goal-row" key={g.id} onClick={()=>{ navigateTo('goals'); setSelectedGoal(g.id) }}><div><b>{g.objective || g.id}</b><span>{g.status || '-'} · {g.running ? `${t.fields.pid} ${g.pid}` : t.fields.notRunning}</span></div><small>{t.fields.turn} {g.turns_used || 0}/{g.max_turns || '-'}</small></button>)
                : <p className="muted">{t.empty}</p>}
            </div>
          </Panel>
          <Panel title={t.nav.autonomous}>
            <div className="actions"><button onClick={()=>navigateTo('autonomous')}><Bot size={14}/>{t.nav.autonomous}</button></div>
