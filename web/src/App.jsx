...
const WikiPage = lazy(() => import('./pages/WikiPage').then(m => ({ default: m.default })))
...
const I18N = {
  zh: {
    ...
    nav: { overview: '总览', chat: '对话', control: '控制面', files: '文件', tasks: '任务', pets: '桌宠', memory: 'llm-wiki', channels: '通道', autonomous: '自主进化', schedule: '定时', goals: 'Goal 模式', models: '模型', settings: '配置', logs: '日志' },
    desc: { overview: '从 GA 的功能域理解并接管生命周期。', chat: '迁移自 reactapp 的 GA 原生对话、文件上传和流式聊天界面。', control: '运行前检查、能力地图、风险摘要与最近报告。', files: '安全浏览 GA 根目录内文本文件，支持 tail 与搜索。', pets: 'GA Admin 内置桌宠工具链：安装 SOP、导出到 GA tools/hatch-pet，不依赖 Codex 目录。', tasks: '普通会话、任务文件、批处理入口、任务型服务与 sche_tasks 定时任务。', memory: 'llm-wiki：同步 GA memory 并 AI 重建知识索引。', channels: '桌面、TUI、Web、IM Bot 等前端入口。', autonomous: '反思、自主运行、Goal Mode 与团队 Worker。', schedule: 'sche_tasks JSON 定时任务详情、编辑、创建与删除。', goals: '复用 GA Goal Mode SOP 与 reflect/goal_mode.py 的持续目标控制台。', models: '读取/预览/写回 GA mykey.py 模型配置。', settings: '配置 GA 根目录、Python、聊天数据目录与 Wiki 目录。', logs: '进程状态与输出日志。' },
...
export default function App() {
  ...
  return <>
    ...
    <main className="main"><header><div><h2>{t.nav[tab]}</h2><p>{t.desc[tab]}</p></div><div className="badges"><span>{cfg?.host}:{cfg?.port}</span><span role="status" aria-live="polite" className={health?.ok?'ok':'err'}>{health?.ok ? t.ready : t.error}</span></div></header>
      ...
      {tab==='pets' && <PetsPage />}
      {tab==='memory' && <WikiPage t={t} cfg={cfg} api={api} />}
